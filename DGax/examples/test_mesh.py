...
# Define boundary condition constants
In = 1
Out = 2
Wall = 3
Far = 4
...
class Elements:
    ...
    def __init__(self, mesh_path, order = 1):
        ...
        Nv, VX, VY, K, EToV, BCType = self.MeshReaderGambitBC2D(mesh_path)
    ...
    def BuildBCMaps2D(Nfp, BCType, vmapM):
        ...
        bc_maps['in'] = np.where(bnodes == In)[0]

        bc_maps['out'] = np.where(bnodes == Out)[0]

        bc_maps['wall'] = np.where(bnodes == Wall)[0]

        bc_maps['far'] = np.where(bnodes == Far)[0]

        bc_maps['cylinder'] = np.where(bnodes == Cyl)[0]

        bc_maps['dirichlet'] = np.where(bnodes == Dirichlet)[0]

        bc_maps['neuman'] = np.where(bnodes == Neuman)[0]

        bc_maps['slip'] = np.where(bnodes == Slip)[0]
...
# 1. read file 
mesh_path = "Mesh_neu/vortexA04.neu"
order = 5

# 3. construct connectivity 
mesh = TriMesh.from_gambit(mesh_path, order)
E = Elements(mesh_path, order)

print("x:", np.array_equal(mesh.x, E.x))
print("y:", np.array_equal(mesh.y, E.y))
print("Fx:", np.array_equal(mesh.Fx, E.Fx))
print("Fy:", np.array_equal(mesh.Fy, E.Fy))
print("Lift:", np.array_equal(mesh.Lift, E.LIFT))
print("vmapM:", np.array_equal(mesh.vmapM, E.vmapM))
print("vmapP:", np.array_equal(mesh.vmapP, E.vmapP))
print("rx:", np.array_equal(mesh.rs_xy[...,0, 0], E.rx))
print("sx:", np.array_equal(mesh.rs_xy[...,0, 1], E.sx))
print("ry:", np.array_equal(mesh.rs_xy[...,1, 0], E.ry))
print("Drw:", np.array_equal(mesh.Dw[..., 0], E.Drw))
print("Dsw:", np.array_equal(mesh.Dw[..., 1], E.Dsw))
print("nx:", np.array_equal(mesh.face_normals[...,0], E.nx))
print("face_scales:", np.array_equal(mesh.face_scale, E.Fscale))
print("mapM:", np.array_equal(mesh.mapM, E.vmapM.reshape(E.Nfp * E.Nfaces, E.K, order='F')))
print("mapP:", np.array_equal(mesh.mapP, E.vmapP.reshape(E.Nfp * E.Nfaces, E.K, order='F')))
# print("BCType:", np.array_equal(mesh.bc_maps, E.bc_maps))
# print("BCType", mesh.bc_maps)
for key in E.bc_maps.keys():
    print(f"Boundary condition '{key}':", np.array_equal(mesh.bc_maps[key], E.bc_maps[key]))
    if not np.array_equal(mesh.bc_maps[key], E.bc_maps[key]):
        print(mesh.bc_maps[key])
        print(E.bc_maps[key])