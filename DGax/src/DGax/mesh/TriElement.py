...
def TriElements(order):
    x, y = Nodes2D(order)
    r, s = xytors(x, y)
    V = Vandermonde2D(order, r, s)
    Dr, Ds = GradVandermonde2D(order, r, s)
    return r, s, V, Dr, Ds
...
def Warpfactor(N, rout):
    ...
    Pmat = np.stack([JacobiP(rout, 0, 0, i).flatten() for i in range(N+1)])
...
def Simplex2DP(a, b, i, j):
    ...
    h1 = JacobiP(a, 0, 0, i).flatten()
    h2 = JacobiP(b, 2*i+1, 0, j).flatten()
...
def GradSimplex2DP(a, b, id, jd):
    ...
    fa = JacobiP(a, 0, 0, id).flatten()
    dfa = GradJacobiP(a, 0, 0, id).flatten()
    gb = JacobiP(b, 2*id+1, 0, jd).flatten()
    dgb = GradJacobiP(b, 2*id+1, 0, jd).flatten()
